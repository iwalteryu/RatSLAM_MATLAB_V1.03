...
PC_DIM_XY = 61;
PC_DIM_TH = 36;
PC_W_E_VAR = 1;             % The variance for the local excitation guassian sphere
PC_W_E_DIM = 7;
...
VTRANS_SCALE = 100;
VISUAL_ODO_SHIFT_MATCH = 400;  %140
IMAGE_VTRANS_Y_RANGE = 1:IMAGE_Y_SIZE; % Row
IMAGE_VROT_Y_RANGE = 1:IMAGE_Y_SIZE;
IMAGE_ODO_X_RANGE = 1:IMAGE_X_SIZE; % column
...
if ODO_FILE ~= 0
    ododata = csvread(ODO_FILE, START_FRAME, 0, [START_FRAME 0 min([(BLOCK_READ - 1)+START_FRAME, FRAME_NUMBER]) 1]);
end

time_delta_s = [];
tic

for frame=2:min([END_FRAME, FRAME_NUMBER])
